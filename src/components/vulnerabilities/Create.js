...
export default function VulnerabilityCreate({ match, history }) {
    ...
    const [vulnerability, setVulnerability] = useState({ projectId: null, summary: null, description: null, risk: Risks[0].id, cvssScore: null })
    ...
    useEffect(function () {
        projects && setVulnerability({ ...vulnerability, projectId: projects[0].id })
    }, [projects]);
    ...
    return (
        <div>
            ...
            <form onSubmit={e => e.preventDefault()}>
                ...
                <select name="projectId" id="projectId" onChange={handleFormChange} defaultValue={projects[0].id} value={vulnerability.projectId}>
                    {projects.map((project, index) =>
                        <option key={index} value={project.id}>{project.name}</option>
                    )}
                </select>
                <label htmlFor='summary'>Summary</label>
                <input autoFocus type="text" name="summary" onChange={handleFormChange} value={vulnerability.summary} />
                <label htmlFor='description'>Description</label>
                <input type="text" name="description" onChange={handleFormChange} value={vulnerability.description} />
                <label htmlFor='risk'>Risk</label>
                <select name="risk" onChange={handleFormChange} defaultValue={Risks[0].id} value={vulnerability.risk}>
